...
public interface PlanRepository extends Neo4jRepository<Plan, Long> {
	...
	@Query(" MATCH (p:Plan)-[a:APPLIES_TO]->(d:Domain) "
			...
			+" 		[(pi)<-[com:COMPOSES]-(pli:PlanItem) | [com,pli]]"
			+" ]")
	List<Plan> findByDomain( @Param("id") Long id);
    ...
    @Query("MATCH () "
            + " OPTIONAL MATCH (plan:Plan) WHERE ext.translate(plan.name) CONTAINS ext.translate($name) "
            + " OPTIONAL MATCH (planItem:PlanItem)-[c:COMPOSES*]->(parentPlan:Plan) WHERE ext.translate(planItem.name) CONTAINS ext.translate($name) "
