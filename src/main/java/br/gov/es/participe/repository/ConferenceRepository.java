...
public interface ConferenceRepository extends Neo4jRepository<Conference, Long> {
  ...
  @Query(
    "MATCH (n:Conference)-[cp:TARGETS]->(p:Plan) "
  ...
  @Query(
    "MATCH (conference:Conference)<-[occurs_in:OCCURS_IN]-(meeting:Meeting) " +
    ...
    "OPTIONAL MATCH (person:Person)-[check_in:CHECKED_IN_AT]->(meeting) " +
    "WHERE ({1} IS NULL) OR ({1} IS NOT NULL AND id(person)={1}) " +
    "RETURN conference, occurs_in, meeting, tpa, locality, person, check_in " +
