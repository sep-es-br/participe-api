...
public interface PlanItemRepository extends Neo4jRepository<PlanItem, Long> {
    ...
    @Query(
        "MATCH (con:Conference)-[trg:TARGETS]->(plan:Plan)" +
        "-[ob:OBEYS]->(stt:Structure)<-[comp:COMPOSES]-(si:StructureItem) " +
        "WHERE id(con)={0} MATCH list=(plan)<-[comp2:COMPOSES*]-(pi2:PlanItem) " +
