...
public interface DomainRepository extends Neo4jRepository<Domain, Long> {
    ...
    @Query("MATCH () "
            + " OPTIONAL MATCH (domain:Domain) WHERE  ext.translate(domain.name) CONTAINS ext.translate($name) "
            + " OPTIONAL MATCH (locality:Locality) WHERE ext.translate(locality.name) CONTAINS ext.translate($name)  "
