...
public interface ControlPanelRepository extends Neo4jRepository<Conference, Long> {
	   ...
	   @Query( " WITH {0} AS Conference_Id,{1} AS SelectedLocality_Id,{2} AS SelectedPlanItem_Id " + 
	   		   ...
	   		   " WHERE ID(conf) = Conference_Id AND(SelectedPlanItem_Id IS NULL OR (id(planItem) = SelectedPlanItem_Id and id(cPI) <> SelectedPlanItem_Id)) " + 
	   		   " OPTIONAL MATCH localized = (p)-[:MADE]->(sd:SelfDeclaration)-[:TO]->(conf), "+
	   		   " (sd)-[:AS_BEING_FROM]->(loc:Locality)-[:IS_LOCATED_IN *0..]->(parentLoc:Locality) " + 
	   		   " WHERE id(parentLoc) = SelectedLocality_Id " + 
	   		   " OPTIONAL MATCH (lp:Person) " + 
	   		   " WHERE lp in nodes(localized) " + 
	   		   " RETURN CASE SelectedPlanItem_Id WHEN NULL THEN id(planItem) ELSE id(cPI) END as idPlanItem, " + 
	   		   " CASE SelectedPlanItem_Id WHEN NULL THEN planItem.name ELSE cPI.name END as planItemName, " + 
	   		   " CASE SelectedLocality_Id WHEN NULL THEN count(distinct p) ELSE count(distinct lp) END as quantityParticipation ")
	   ...
	   @Query(  " WITH {0} AS Conference_Id,{1} AS SelectedLocality_Id,{2} AS SelectedPlanItem_Id " + 
	   		    " MATCH(p:Person)<-[:MADE_BY|:LIKED_BY]-(a:Attend)-[:ABOUT]->(cPI:PlanItem), " + 
	   		    " (cPI)-[:COMPOSES *0..]->(planItem:PlanItem)-[:COMPOSES]->(plan:Plan)<-[:TARGETS]-(conf:Conference) " + 
	   		    " WHERE ID(conf) = Conference_Id AND a.from = 'rem' AND(SelectedPlanItem_Id IS NULL OR " + 
		   		...
		   		" OPTIONAL MATCH (lp:Person) " + 
		   		" WHERE lp in nodes(localized) " + 
		   		" WITH lp,p,planItem,cPI,SelectedLocality_Id,SelectedPlanItem_Id " + 
		   		" RETURN CASE SelectedPlanItem_Id WHEN NULL THEN id(planItem) ELSE id(cPI) END as idPlanItem, " + 
		   		" CASE SelectedPlanItem_Id WHEN NULL THEN planItem.name ELSE cPI.name END as planItemName, " + 
		   		" case SelectedLocality_Id WHEN NULL THEN count(distinct p) ELSE count(distinct lp) END as quantityParticipation ")
	   ...
	   @Query( " WITH {0} AS Conference_Id,{1} AS SelectedLocality_Id,{2} AS SelectedPlanItem_Id,{3} AS Meeting_List " + 
	   		   " MATCH(p:Person)<-[:MADE_BY|:LIKED_BY]-(a:Attend)-[:ABOUT]->(cPI:PlanItem), " + 
	   		   " (cPI)-[:COMPOSES *0..]->(planItem:PlanItem)-[:COMPOSES]->(plan:Plan)<-[:TARGETS]-(conf:Conference)<-[:OCCURS_IN]-(me:Meeting) " + 
	   		   " WHERE ID(conf) = Conference_Id AND a.from = 'pres' AND (Meeting_List IS NULL OR " + 
	   		   " id(me) IN Meeting_List) AND(SelectedPlanItem_Id IS NULL OR (id(planItem) = SelectedPlanItem_Id and id(cPI) <> SelectedPlanItem_Id)) " + 
	   		   " OPTIONAL MATCH localized = (p)-[:MADE]->(sd:SelfDeclaration)-[:TO]->(conf), " + 
	   		   " (sd)-[:AS_BEING_FROM]->(loc:Locality)-[:IS_LOCATED_IN *0..]->(parentLoc:Locality) " + 
	   		   " WHERE id(parentLoc) = SelectedLocality_Id " + 
	   		   " OPTIONAL MATCH (lp:Person) " + 
	   ...
	   @Query(  " WITH {0} AS Conference_Id,{1} AS SelectedLocality_Id,{2} AS SelectedPlanItem_Id" + 
	   		    " MATCH (a:Highlight)-[:ABOUT]->(cPI:PlanItem)-[:COMPOSES *0..]->(planItem:PlanItem)-[:COMPOSES]->(plan:Plan)<-[:TARGETS]-(conf:Conference) " + 
	   		    " WHERE ID(conf) = Conference_Id AND(SelectedPlanItem_Id IS NULL OR (id(planItem) = SelectedPlanItem_Id and id(cPI) <> SelectedPlanItem_Id)) " + 
	   		    " OPTIONAL MATCH localized = (a)-[:ABOUT]->(loc:Locality)-[:IS_LOCATED_IN *0..]->(parentLoc:Locality) " + 
	   		    " WHERE id(parentLoc) = SelectedLocality_Id " + 
	   		    " OPTIONAL MATCH (la:Highlight) " + 
	   		    " WHERE la in nodes(localized) " + 
	   		    " WITH la,a,planItem,cPI,SelectedLocality_Id,SelectedPlanItem_Id " + 
	   		    " RETURN CASE SelectedPlanItem_Id WHEN NULL THEN id(planItem) ELSE id(cPI) END as idPlanItem, " + 
	   		    " CASE SelectedPlanItem_Id WHEN NULL THEN planItem.name ELSE cPI.name END as planItemName, " + 
	   		    " CASE SelectedLocality_Id WHEN NULL THEN count(distinct a) ELSE count(distinct la) END as quantityHighlight ")
	   ...
	   @Query( " WITH {0} AS Conference_Id,{1} AS SelectedLocality_Id,{2} AS SelectedPlanItem_Id,{3} AS Meeting_List " + 
	   		   " MATCH (a:Highlight)-[:ABOUT]->(cPI:PlanItem)-[:COMPOSES *0..]->(planItem:PlanItem)-[:COMPOSES]->(plan:Plan)<-[:TARGETS]-(conf:Conference)<-[:OCCURS_IN]-(me:Meeting) " + 
	   	       " WHERE ID(conf) = Conference_Id AND(SelectedPlanItem_Id IS NULL OR " + 
	   		   " (id(planItem) = SelectedPlanItem_Id and id(cPI) <> SelectedPlanItem_Id)) AND a.from = 'pres' " + 
	   		   " AND (Meeting_List IS NULL OR id(me) IN Meeting_List) " + 
	   		   " OPTIONAL MATCH localized = (a)-[:AS_BEING_FROM]->(loc:Locality)-[:IS_LOCATED_IN *0..]->(parentLoc:Locality) " + 
	   		   " WHERE id(parentLoc) = SelectedLocality_Id " + 
	   		   " OPTIONAL MATCH (la:Highlight) " + 
	   		   " WHERE la in nodes(localized) " + 
	   		   " WITH la,a,planItem,cPI,SelectedLocality_Id,SelectedPlanItem_Id " + 
	   ...
	   @Query( " WITH {0} AS Conference_Id,{1} AS SelectedLocality_Id,{2} AS SelectedPlanItem_Id " + 
	   		   " MATCH (a:Comment)-[:ABOUT]->(cPI:PlanItem)-[:COMPOSES *0..]->(planItem:PlanItem)-[:COMPOSES]->(plan:Plan)<-[:TARGETS]-(conf:Conference) " + 
	   		   " WHERE ID(conf) = Conference_Id AND a.status IN ['pub', 'arq'] " + 
	   		   " AND(SelectedPlanItem_Id IS NULL OR (id(planItem) = SelectedPlanItem_Id and id(cPI) <> SelectedPlanItem_Id)) " + 
	   		   " OPTIONAL MATCH localized = (a)-[:ABOUT]->(loc:Locality)-[:IS_LOCATED_IN *0..]->(parentLoc:Locality) " + 
	   		   " WHERE id(parentLoc) = SelectedLocality_Id " + 
	   		   " OPTIONAL MATCH (la:Comment) " + 
	   		   " WHERE la in nodes(localized) " + 
	   		   " WITH la,a,planItem,cPI,SelectedLocality_Id,SelectedPlanItem_Id " + 
	   		   " RETURN CASE SelectedPlanItem_Id WHEN NULL THEN id(planItem) ELSE id(cPI) END as idPlanItem, " + 
	   		   " CASE SelectedPlanItem_Id WHEN NULL THEN planItem.name ELSE cPI.name END as planItemName, " + 
	   		   " CASE SelectedLocality_Id WHEN NULL THEN count(distinct a) ELSE count(distinct la) END as quantityComment ")
	   ...
	   @Query( " WITH {0} AS Conference_Id,{1} AS SelectedLocality_Id,{2} AS SelectedPlanItem_Id " + 
	   		   " MATCH (a:Comment)-[:ABOUT]->(cPI:PlanItem)-[:COMPOSES *0..]->(planItem:PlanItem)-[:COMPOSES]->(plan:Plan)<-[:TARGETS]-(conf:Conference) " + 
	   		   " WHERE ID(conf) = Conference_Id AND a.status IN ['pub', 'arq'] AND	a.from = 'rem' " + 
	   		   " AND(SelectedPlanItem_Id IS NULL OR (id(planItem) = SelectedPlanItem_Id and id(cPI) <> SelectedPlanItem_Id)) " + 
	   		   " OPTIONAL MATCH localized = (a)-[:ABOUT]->(loc:Locality)-[:IS_LOCATED_IN *0..]->(parentLoc:Locality) " + 
	   		   " WHERE id(parentLoc) = SelectedLocality_Id " + 
	   		   " OPTIONAL MATCH (la:Comment) " + 
	   ...
	   @Query( " WITH {0} AS Conference_Id,{1} AS SelectedLocality_Id,{2} AS SelectedPlanItem_Id,{3} AS Meeting_List " + 
	   		   " MATCH (a:comment)-[:ABOUT]->(cPI:PlanItem)-[:COMPOSES *0..]->(planItem:PlanItem)-[:COMPOSES]->(plan:Plan)<-[:TARGETS]-(conf:Conference)<-[:OCCURS_IN]-(me:Meeting) " + 
	   		   " WHERE ID(conf) = Conference_Id AND(SelectedPlanItem_Id IS NULL " + 
	   		   " OR (id(planItem) = SelectedPlanItem_Id and id(cPI) <> SelectedPlanItem_Id)) " + 
	   		   " AND a.from = 'pres' AND a.status IN ['pub', 'arq'] AND (Meeting_List IS NULL " + 
	   		   " OR id(me) IN Meeting_List) " + 
	   		   " OPTIONAL MATCH localized = (a)-[:AS_BEING_FROM]->(loc:Locality)-[:IS_LOCATED_IN *0..]->(parentLoc:Locality) " + 
	   		   " WHERE id(parentLoc) = SelectedLocality_Id " + 
	   		   " OPTIONAL MATCH (la:Comment) " + 
	   		   " WHERE la in nodes(localized) " + 
