...
public class ProposalsService {
  ...
  private static final Logger log = LoggerFactory.getLogger(ProposalsService.class);
	...
	public Integer makeLike(Long idPerson, Long idComment) {
        ...
        log.info(
          "Criando relação entre personId={} e commentId={}",
          person.getId(),
          comment.getId()
        );
        ...
        log.info(
          "Removendo relação entre personId={} e commentId={}",
          person.getId(),
          comment.getId()
        );
				personService.save(person, true);
